...
static qboolean S_VORBIS_CodecOpenStream(snd_stream_t* stream)
{
    ...
    return true;
_fail:
    if(res == 0) ov_clear(ovFile);
    Z_Free(ovFile);
    return false;
}

static int S_VORBIS_CodecReadStream(
...
snd_codec_t vorbis_codec = {CODECTYPE_VORBIS, true, /* always available. */
    "ogg", S_VORBIS_CodecInitialize, S_VORBIS_CodecShutdown,
    S_VORBIS_CodecOpenStream, S_VORBIS_CodecReadStream,
    S_VORBIS_CodecRewindStream, S_VORBIS_CodecCloseStream, nullptr};
