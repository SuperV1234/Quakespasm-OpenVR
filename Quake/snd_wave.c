...
static int WAV_FindRIFFChunk(FILE* f, const char* chunk)
{
    ...
    while((len = WAV_ReadChunkInfo(f, name)) >= 0)
    {
        ...
        if(!strncmp(name, chunk, 4)) return len;
...
=================
*/
static qboolean WAV_ReadRIFFHeader(
...
snd_codec_t wav_codec = {CODECTYPE_WAVE, true, /* always available. */
    "wav", S_WAV_CodecInitialize, S_WAV_CodecShutdown, S_WAV_CodecOpenStream,
    S_WAV_CodecReadStream, S_WAV_CodecRewindStream, S_WAV_CodecCloseStream,
    nullptr};
