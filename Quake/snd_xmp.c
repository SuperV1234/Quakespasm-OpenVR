...
static int S_XMP_StartPlay(snd_stream_t* stream)
{
    ...
    if(stream->info.channels == 1) fmt |= XMP_FORMAT_MONO;
    if(stream->info.width == 1) fmt |= XMP_FORMAT_8BIT | XMP_FORMAT_UNSIGNED;
...
static qboolean S_XMP_CodecOpenStream(snd_stream_t* stream)
{
    ...
    if(S_XMP_StartPlay(stream) != 0) goto err2;
    /* percentual left/right channel separation, default is 70. */
    if(stream->info.channels == 2)
        if(xmp_set_player(c, XMP_PLAYER_MIX, 100) != 0) goto err3;
    /* interpolation type, default is XMP_INTERP_LINEAR */
    if(xmp_set_player(c, XMP_PLAYER_INTERP, XMP_INTERP_SPLINE) != 0) goto err3;
...
snd_codec_t xmp_codec = {CODECTYPE_MOD, true, /* always available. */
    "s3m", S_XMP_CodecInitialize, S_XMP_CodecShutdown, S_XMP_CodecOpenStream,
    S_XMP_CodecReadStream, S_XMP_CodecRewindStream, S_XMP_CodecCloseStream,
    nullptr};
