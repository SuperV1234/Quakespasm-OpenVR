...
void Con_ToggleConsole_f()
{
    if(key_dest == key_console /* || (key_dest == key_game && con_forcedup)*/)
    {
        ...
        key_linepos = 1;
        con_backscroll = 0; // johnfitz -- toggleconsole should return you to
                            // the bottom of the scrollback
        history_line = edit_line; // johnfitz -- it should also return you to
                                  // the bottom of the command history
...
static const arg_completion_type_t arg_completion_types[] = {
    {"map ", &extralevels}, {"changelevel ", &extralevels}, {"game ", &modlist},
...
void Con_TabComplete()
{
    ...
    for(j = 0; j < num_arg_completion_types; j++)
    {
        ...
        if(!strncmp(
               key_lines[edit_line] + 1, command_name, strlen(command_name)))
        {
            int nummatches = 0;
...
void Con_DrawInput()
{
    ...
    if(!((int)((realtime - key_blinktime) * con_cursorspeed) & 1))
    {
        i = key_linepos - ofs;
        Draw_Pic(
            (i + 1) << 3, vid.conheight - 16, key_insert ? pic_ins : pic_ovr);
    }
}
...
void LOG_Init(quakeparms_t* parms)
{
    ...
    strftime(
        session, sizeof(session), "%m/%d/%Y %H:%M:%S", localtime(&inittime));
    q_snprintf(
