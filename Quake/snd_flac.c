...
static FLAC__StreamDecoderReadStatus flac_read_func(
    const FLAC__StreamDecoder* decoder, FLAC__byte buffer[], FLAC_SIZE_T* bytes,
    void* client_data)
{
    flacfile_t* ff = (flacfile_t*)client_data;
    if(*bytes > 0)
    {
        *bytes = FS_fread(buffer, 1, *bytes, ff->file);
        if(FS_ferror(ff->file)) return FLAC__STREAM_DECODER_READ_STATUS_ABORT;
...
static FLAC__bool flac_eof_func(
    ...
    const FLAC__int32* const buffer[], void* client_data)
{
    ...
    if(!ff->buffer)
    {
        ff->buffer = (byte*)malloc(
            ff->info->blocksize * ff->info->channels * ff->info->width);
        if(!ff->buffer)
        {
    ...
    if(ff->info->channels == 1)
    {
        ...
        {
            byte* out = ff->buffer;
            for(i = 0; i < frame->header.blocksize; i++) *out++ = *in++ + 128;
        }
        else
        {
            short* out = (short*)ff->buffer;
            for(i = 0; i < frame->header.blocksize; i++) *out++ = *in++;
        }
    }
    else
    {
...
static qboolean S_FLAC_CodecOpenStream(snd_stream_t* stream)
{
    ...
    if(rc == false || ff->error)
    {
        ...
        Con_Printf(
            "%s not a valid flac file? (decoder state %i)\n", stream->name, rc);
        goto _fail;
    }
...
snd_codec_t flac_codec = {CODECTYPE_FLAC, true, /* always available. */
    "flac", S_FLAC_CodecInitialize, S_FLAC_CodecShutdown,
    S_FLAC_CodecOpenStream, S_FLAC_CodecReadStream, S_FLAC_CodecRewindStream,
    S_FLAC_CodecCloseStream, nullptr};
