...
static off_t mp3_seek(void* f, off_t offset, int whence)
{
    ...
    if(FS_fseek((fshandle_t*)f, (long)offset, whence) < 0) return (off_t)-1;
...
static qboolean S_MP3_CodecOpenStream(snd_stream_t* stream)
{
    ...
    if(mpg123_replace_reader_handle(
           priv->handle, mp3_read, mp3_seek, nullptr) != MPG123_OK ||
    ...
    switch(channels)
    {
        case MPG123_MONO: stream->info.channels = 1; break;
        case MPG123_STEREO: stream->info.channels = 2; break;
        default:
...
_fail:
    if(priv)
    {
        if(priv->handle)
        {
            if(priv->handle_open) mpg123_close(priv->handle);
...
static int S_MP3_CodecReadStream(snd_stream_t* stream, int bytes, void* buffer)
{
    ...
    size_t bytes_read = 0;
    int res = mpg123_read(
        priv->handle, (unsigned char*)buffer, (size_t)bytes, &bytes_read);
    switch(res)
    {
        case MPG123_DONE: Con_DPrintf("mp3 EOF\n");
        case MPG123_OK: return (int)bytes_read;
    }
    return -1; /* error */
}
...
snd_codec_t mp3_codec = {CODECTYPE_MP3, false, "mp3", S_MP3_CodecInitialize,
    S_MP3_CodecShutdown, S_MP3_CodecOpenStream, S_MP3_CodecReadStream,
    S_MP3_CodecRewindStream, S_MP3_CodecCloseStream, nullptr};
