...
void Sky_NewMap(void)
{
    ...
    //
    skybox_name[0] = 0;
    for(i = 0; i < 6; i++) skybox_textures[i] = nullptr;
    skyfog = r_skyfog.value;

    //
    // read worldspawn (this is so ugly, and shouldn't it be done on the
    // server?)
    //
...
void Sky_SkyCommand_f(void)
{
    switch(Cmd_Argc())
    {
        case 1: Con_Printf("\"sky\" is \"%s\"\n", skybox_name); break;
        case 2: Sky_LoadSkyBox(Cmd_Argv(1)); break;
        default: Con_Printf("usage: sky <skyname>\n");
    }
}
...
void Sky_ClipPoly(int nump, vec3_t vecs, int stage)
{
    ...
    if(nump > MAX_CLIP_VERTS - 2) Sys_Error("Sky_ClipPoly: MAX_CLIP_VERTS");
    ...
    for(i = 0; i < cl_numvisedicts; i++)
    {
        ...
        for(j = 0; j < e->model->nummodelsurfaces; j++, s++)
        {
            if(s->flags & SURF_DRAWSKY)
            {
                ...
                {
                    // copy the polygon and translate manually, since
                    // Sky_ProcessPoly needs it to be in world space
                    mark = Hunk_LowMark();
                    p = (glpoly_t*)Hunk_Alloc(sizeof(
                        *s->polys)); // FIXME: don't allocate for each poly
                    ...
                    for(k = 0; k < p->numverts; k++)
                    {
                        if(rotated)
                        {
                            ...
                            VectorAdd(
                                s->polys->verts[k], e->origin, p->verts[k]);
                    }
                    Sky_ProcessPoly(p);
                    Hunk_FreeToLowMark(mark);
                }
...
void Sky_DrawFaceQuad(glpoly_t* p)
{
    ...
    else
    {
        ...
        if(r_skyalpha.value < 1.0) glColor3f(1, 1, 1);
        ...
        if(r_skyalpha.value < 1.0) glColor4f(1, 1, 1, r_skyalpha.value);
    ...
    if(Fog_GetDensity() > 0 && skyfog > 0)
    {
        ...
        glBegin(GL_QUADS);
        for(i = 0, v = p->verts[0]; i < 4; i++, v += VERTEXSIZE) glVertex3fv(v);
        glEnd();
...
void Sky_DrawSky(void)
{
    ...
    if(r_drawflat_cheatsafe || r_lightmap_cheatsafe) return;
    ...
    //
    // process world and bmodels: draw flat-shaded sky surfs, and update
    // skybounds
    //
    Fog_DisableGFog();
    glDisable(GL_TEXTURE_2D);
