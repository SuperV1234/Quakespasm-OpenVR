...
keyname_t keynames[] = {{"TAB", K_TAB}, {"ENTER", K_ENTER},
    {"ESCAPE", K_ESCAPE}, {"SPACE", K_SPACE}, {"BACKSPACE", K_BACKSPACE},
    ...
    {"KP_SLASH", K_KP_SLASH}, {"KP_STAR", K_KP_STAR}, {"KP_MINUS", K_KP_MINUS},
    {"KP_HOME", K_KP_HOME}, {"KP_UPARROW", K_KP_UPARROW},
    {"KP_PGUP", K_KP_PGUP}, {"KP_PLUS", K_KP_PLUS},
    {"KP_LEFTARROW", K_KP_LEFTARROW}, {"KP_5", K_KP_5},
    {"KP_RIGHTARROW", K_KP_RIGHTARROW}, {"KP_END", K_KP_END},
    {"KP_DOWNARROW", K_KP_DOWNARROW}, {"KP_PGDN", K_KP_PGDN},
    ...
    {"AUX9", K_AUX9}, {"AUX10", K_AUX10}, {"AUX11", K_AUX11},
    {"AUX12", K_AUX12}, {"AUX13", K_AUX13}, {"AUX14", K_AUX14},
    {"AUX15", K_AUX15}, {"AUX16", K_AUX16}, {"AUX17", K_AUX17},
    {"AUX18", K_AUX18}, {"AUX19", K_AUX19}, {"AUX20", K_AUX20},
    {"AUX21", K_AUX21}, {"AUX22", K_AUX22}, {"AUX23", K_AUX23},
    {"AUX24", K_AUX24}, {"AUX25", K_AUX25}, {"AUX26", K_AUX26},
    {"AUX27", K_AUX27}, {"AUX28", K_AUX28}, {"AUX29", K_AUX29},
    {"AUX30", K_AUX30}, {"AUX31", K_AUX31}, {"AUX32", K_AUX32},
    ...
    {"LTHUMB", K_LTHUMB}, {"RTHUMB", K_RTHUMB}, {"LSHOULDER", K_LSHOULDER},
    {"RSHOULDER", K_RSHOULDER}, {"ABUTTON", K_ABUTTON}, {"BBUTTON", K_BBUTTON},
    {"XBUTTON", K_XBUTTON}, {"YBUTTON", K_YBUTTON}, {"LTRIGGER", K_LTRIGGER},
    {"RTRIGGER", K_RTRIGGER},
...
static void PasteToConsole()
{
    ...
    if((cbd = PL_GetClipboardData()) == nullptr) return;
...
void Key_Console(int key)
{
    ...
    switch(key)
    {
        ...
        case K_KP_ENTER:
            ...
            // If the last two lines are identical, skip storing this line in
            // history by not incrementing edit_line
            if(strcmp(workline, key_lines[(edit_line - 1) & 31]))
                edit_line = (edit_line + 1) & 31;

            history_line = edit_line;
            key_lines[edit_line][0] = ']';
            key_lines[edit_line][1] = 0; // johnfitz -- otherwise old history
                                         // items show up in the new edit line
            key_linepos = 1;
            if(cls.state == ca_disconnected)
                SCR_UpdateScreen(); // force an update, because the command may
                                    // take some time
            return;

        case K_TAB: Con_TabComplete(); return;
        ...
        case K_RIGHTARROW:
            ...
            if((int)len == key_linepos)
            {
                ...
                if((int)len <= key_linepos) return; // no character to get
                workline += key_linepos;
        ...
        case K_UPARROW:
            if(history_line == edit_line) Q_strcpy(current, workline);
...
void Key_Message(int key)
{
    switch(key)
    {
        ...
        case K_ESCAPE: Key_EndChat(); return;
...
void Key_Unbindall_f()
{
    ...
    {
        if(keybindings[i]) Key_SetBinding(i, nullptr);
    }
}
...
{
    if(lastkey) *lastkey = key_inputgrab.lastkey;
    if(lastchar) *lastchar = key_inputgrab.lastchar;
}
...
void Key_Event(int key, qboolean down)
{
    ...
    {
        if(down) key_inputgrab.lastkey = key;
        return;
    }
    ...
    // a button command (leading + sign).  These will occur even in console
...
void Char_Event(int key)
{
...
#if defined(PLATFORM_OSX) || defined(PLATFORM_MAC)
    if(keydown[K_COMMAND]) return;
#endif
    if(keydown[K_CTRL]) return;
    ...
    switch(key_dest)
    {
        case key_message: Char_Message(key); break;
        case key_menu: M_Charinput(key); break;
        case key_game:
...
qboolean Key_TextEntry()
{
    ...
    switch(key_dest)
    {
        case key_message: return true;
        case key_menu: return M_TextEntry();
        case key_game:
