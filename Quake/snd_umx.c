...
_retry:
    ...
    if(type == UMUSIC_IT)
    {
        if(memcmp(sig, "IMPM", 4) == 0) return UMUSIC_IT;
        return -1;
    }
    if(type == UMUSIC_XM)
    {
        if(memcmp(sig, "Extended Module:", 16) != 0) return -1;
        FS_fread(sig, 16, 1, f);
        if(sig[0] != ' ') return -1;
        FS_fread(sig, 16, 1, f);
    ...
    if(type == UMUSIC_MP2)
    {
        ...
        if(u == 0xFFFC || u == 0xFFF4) return UMUSIC_MP2;
        return -1;
    }
    if(type == UMUSIC_WAV)
    {
    ...
    if(type == UMUSIC_S3M)
    {
        if(memcmp(sig, "SCRM", 4) == 0) return UMUSIC_S3M;
        /*return -1;*/
...
static int read_export(
    fshandle_t* f, const struct upkg_hdr* hdr, int32_t* ofs, int32_t* objsize)
{
    char buf[40];
    int idx = 0, t;

    FS_fseek(f, *ofs, SEEK_SET);
    if(FS_fread(buf, 4, 10, f) < 10) return -1;
...
static int read_typname(
    fshandle_t* f, const struct upkg_hdr* hdr, int idx, char* out)
{
    int i, s;
    long l;
    char buf[64];
...
static int32_t probe_header(void* header)
{
    ...
    if(hdr->name_count < 0 || hdr->name_offset < 0 || hdr->export_count < 0 ||
        hdr->export_offset < 0 || hdr->import_count < 0 ||
        hdr->import_offset < 0)
    {
    ...
    switch(hdr->file_version)
    {
        ...
        case 83: /* Mobile Forces */ return 0;
    }
...
static int process_upkg(fshandle_t* f, int32_t* ofs, int32_t* objsize)
{
    ...
    if(FS_fread(header, 1, UPKG_HDR_SIZE, f) < UPKG_HDR_SIZE) return -1;
    if(probe_header(header) < 0) return -1;
...
snd_codec_t umx_codec = {CODECTYPE_UMX, true, /* always available. */
    "umx", S_UMX_CodecInitialize, S_UMX_CodecShutdown, S_UMX_CodecOpenStream,
    S_UMX_CodecReadStream, S_UMX_CodecRewindStream, S_UMX_CodecCloseStream,
    nullptr};
