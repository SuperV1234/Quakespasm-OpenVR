...
sys_socket_t UDP_Init()
{
    ...
    else
    {
        ...
        // don't bother calling gethostbyname(), because it blocks for a few
        // seconds and then fails (on my system anyway.)
...
int UDP_Write(sys_socket_t socketid, byte* buf, int len, struct qsockaddr* addr)
{
    ...
    ret = sendto(socketid, reinterpret_cast<char*>(buf), len, 0,
