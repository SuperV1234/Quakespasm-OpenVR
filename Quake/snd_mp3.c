...
static int mp3_startread(snd_stream_t* stream)
{
    ...
    if(!ReadSize || FS_ferror(&stream->fh)) return -1;
    ...
    while(1) /* Read data from the MP3 file */
    {
        ...
        while(1) /* Decode frame headers */
        {
            ...
            if(p->FrameCount == 64 && !vbr)
            {
                ...
                if(0 != FS_fseek(&stream->fh, (p->FrameCount * consumed / 64),
                            SEEK_SET))
                    return -1;
...
snd_codec_t mp3_codec = {CODECTYPE_MP3, true, /* always available. */
    "mp3", S_MP3_CodecInitialize, S_MP3_CodecShutdown, S_MP3_CodecOpenStream,
    S_MP3_CodecReadStream, S_MP3_CodecRewindStream, S_MP3_CodecCloseStream,
    nullptr};
