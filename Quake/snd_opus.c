...
static qboolean S_OPUS_CodecOpenStream(snd_stream_t* stream)
{
    ...
    {
        Con_Printf(
            "%s is not a valid Opus file (error %i).\n", stream->name, res);
        goto _fail;
    }
...
static int S_OPUS_CodecReadStream(snd_stream_t* stream, int bytes, void* buffer)
{
    ...
    if(rem / stream->info.channels <= 0) return 0;
    ...
    while(1)
    {
         ...
         * the number of samples read per channel on success, or a negative
         * value on failure. */
...
snd_codec_t opus_codec = {CODECTYPE_OPUS, true, /* always available. */
    "opus", S_OPUS_CodecInitialize, S_OPUS_CodecShutdown,
    S_OPUS_CodecOpenStream, S_OPUS_CodecReadStream, S_OPUS_CodecRewindStream,
    S_OPUS_CodecCloseStream, nullptr};
