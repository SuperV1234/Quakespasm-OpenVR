...
void Cbuf_Init(void)
{
    SZ_Alloc(&cmd_text,
        1 << 18); // space for commands and script files. spike -- was 8192, but
                  // modern configs can be _HUGE_, at least if they contain lots
...
void Cmd_Unalias_f(void)
{
    ...
    {
        default:
        case 1: Con_Printf("unalias <name> : delete alias\n"); break;
        case 2:
            prev = nullptr;
            for(a = cmd_alias; a; a = a->next)
            {
...
static char* Cmd_TintSubstring(
    const char* in, const char* substr, char* out, size_t outsize)
{
    int l;
    char* m;
