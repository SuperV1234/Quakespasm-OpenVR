...
static long get_ape_len(const unsigned char* data)
{
    ...
    long size = (long)((data[15] << 24) | (data[14] << 16) | (data[13] << 8) |
                       data[12]);
...
static long get_lyrics3v1_len(snd_stream_t* stream)
{
    ...
    {
        if(memcmp(p, "LYRICSBEGIN", 11) == 0) break;
    }
    if(i < 0) return -1;
...
static long get_musicmatch_len(snd_stream_t* stream)
{
    ...
    const unsigned char syncstr[10] = {
        '1', '8', '2', '7', '3', '6', '4', '5', 0, 0};
    ...
    imgext_ofs =
        (int)((buf[3] << 24) | (buf[2] << 16) | (buf[1] << 8) | buf[0]);
    version_ofs =
...
static int probe_id3v1(snd_stream_t* stream, unsigned char* buf, int atend)
{
    if(stream->fh.length >= 128)
    {
        ...
        if(FS_fread(buf, 1, 128, &stream->fh) != 128) return -1;
        if(is_id3v1(buf, 128))
        {
            if(!atend)
...
static int probe_mmtag(snd_stream_t* stream, unsigned char* buf)
{
    ...
    if(stream->fh.length >= 68)
    {
        ...
        if(FS_fread(buf, 1, 48, &stream->fh) != 48) return -1;
...
static int probe_apetag(snd_stream_t* stream, unsigned char* buf)
{
    ...
    if(stream->fh.length >= 32)
    {
        ...
        if(FS_fread(buf, 1, 32, &stream->fh) != 32) return -1;
        if(is_apetag(buf, 32))
        {
            len = get_ape_len(buf);
            if(len >= stream->fh.length) return -1;
...
static int probe_lyrics3(snd_stream_t* stream, unsigned char* buf)
{
    ...
    if(stream->fh.length >= 15)
    {
        ...
        if(FS_fread(buf, 1, 15, &stream->fh) != 15) return -1;
        ...
        if(len == 2)
        {
            ...
            if(len < 15) return -1;
            FS_fseek(&stream->fh, -len, SEEK_END);
            if(FS_fread(buf, 1, 11, &stream->fh) != 11) return -1;
...
int mp3_skiptags(snd_stream_t* stream)
{
    ...
    for(;;)
    {
        if(!c_lyr)
        {
            ...
            if((c_lyr = probe_lyrics3(stream, buf)) < 0) goto fail;
            if(c_lyr) continue;
        }
        if(!c_mm)
        {
            if((c_mm = probe_mmtag(stream, buf)) < 0) goto fail;
            if(c_mm) continue;
        }
        if(!c_ape)
        {
            if((c_ape = probe_apetag(stream, buf)) < 0) goto fail;
            if(c_ape) continue;
        }
        break;
    } /* for (;;) */
