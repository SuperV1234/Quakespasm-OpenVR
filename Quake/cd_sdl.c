...
static int CDAudio_GetAudioDiskInfo(void)
{
    ...
    if(!cd_handle) return -1;

    if(!CD_INDRIVE(SDL_CDStatus(cd_handle))) return -1;
...
void CDAudio_Stop(void)
{
    ...
    if(SDL_CDPause(cd_handle) < 0)
        Con_Printf(
            "CDAudio_Stop: Unable to stop CD-ROM (%s)\n", SDL_GetError());
#else
...
static int get_first_audiotrk(void)
{
    ...
    for(i = 0; i < cd_handle->numtracks; ++i)
        if(cd_handle->track[i].type != SDL_DATA_TRACK) return ++i;
    return 1;
}
...
void CDAudio_Update(void)
{
    ...
    if(old_cdvolume != bgmvolume.value) CDAudio_SetVolume(bgmvolume.value);
    ...
    if(playing && realtime > endOfTrack)
    {
        ...
        if(curstat != CD_PLAYING && curstat != CD_PAUSED)
        {
            playing = false;
            endOfTrack = -1.0;
            if(playLooping) CDAudio_Play(playTrack, true);
        }
    }
}
...
static const char* get_cddev_arg(const char* arg)
{
    ...
    if(arg[1] != '\0')
    {
        ...
        if(arg[2] != '\0')
        {
            if(arg[2] != '\\' && arg[2] != '/') return nullptr;
            if(arg[3] != '\0') return nullptr;
        }
    }
    if(*arg >= 'A' && *arg <= 'Z')
    {
...
int CDAudio_Init(void)
{
    ...
    if(safemode || COM_CheckParm("-nocdaudio")) return -1;
    ...
    if(cd_dev == -1) cd_dev = 0; /* default drive */
    ...
    if(hw_vol_works) hw_vol_works = CDAudio_SetVolume(bgmvolume.value);
...
void CDAudio_Shutdown(void)
{
    if(!cd_handle) return;
    CDAudio_Stop();
    if(hw_vol_works) CD_SetVolume(nullptr); /* no SDL support at present. */
#ifdef __linux__
    SDL_CDStop(cd_handle);                  /* see CDAudio_Stop() */
#endif
